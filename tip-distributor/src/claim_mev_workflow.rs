use {
    crate::{
        ...
        send_until_blockhash_expires, GeneratedMerkleTreeCollection,
    },
    itertools::Itertools,
    log::{info, warn},
    ...
    for tree in &merkle_trees.generated_merkle_trees {
        ...
        for node in &tree.tree_nodes {
            ...
            instructions.push(Instruction {
                ...
                data: jito_sdk::instruction::Claim {
                    proof: node.proof.clone().unwrap(),
                    amount: node.amount,
                    bump: node.claim_status_bump,
                }
                .data(),
                accounts: jito_sdk::accounts::Claim {
                    config: tip_distribution_config,
                    tip_distribution_account: tree.tip_distribution_account,
                    // Claims are permissioned: the merkle_root_upload_authority must sign. The
                    // claim payer keypair IS the upload authority, so reuse payer_pubkey here
                    // (one signature satisfies both signer slots).
                    merkle_root_upload_authority: payer_pubkey,

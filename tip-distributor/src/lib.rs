...
use {
    crate::{
        jito_sdk::{
            ...
            tip_payment::{
                Config, CONFIG_ACCOUNT_SEED, TIP_ACCOUNT_SEED_0, TIP_ACCOUNT_SEED_1,
...
/// Error type for merkle-root generation. Previously lived in the now-removed
/// `merkle_root_generator_workflow` module (which depended on anchor); folded in here.
...
async fn signed_send(
    ...
    let res = match rpc_client.send_and_confirm_transaction(&txn).await {
        Ok(_) => Ok(()),
        Err(e) => {
            // `RpcSimulateTransactionResult::err` is now `Option<UiTransactionError>`, whose
            // inner `TransactionError` field is private; convert via `Into` to inspect it.
            let preflight_inner_err = if let ErrorKind::RpcError(RpcError::RpcResponseError {
                data:
            ...
            match &*e.kind {
                ...
                ErrorKind::RpcError(RpcError::RpcResponseError {
                    ...
                    preflight_inner_err,
                    Some(TransactionError::InstructionError(0, InstructionError::Custom(0)))
                ) =>
                {
                    Ok(())
                }
...
/// `solana-rent` 4.1 dropped these public constants; mirror the historical default values
/// (`Rent::default()`), matching the original upstream `minimum_balance` math.

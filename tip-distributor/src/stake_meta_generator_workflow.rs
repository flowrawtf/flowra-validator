use {
    crate::{
        ...
        jito_sdk::AccountDeserialize, Config, StakeMeta, StakeMetaCollection,
    ...
    itertools::Itertools,
    log::*,
    solana_account::{from_account, ReadableAccount, WritableAccount},
    solana_accounts_db::utils::create_all_accounts_run_and_snapshot_dirs,
    ...
    solana_ledger::{
        ...
        blockstore_processor::{
            self, BlockstoreProcessorError, ProcessOptions,
        },
...
fn create_bank_from_snapshot(
    ...
    // Accounts must live under run/ + snapshot/ sub-dirs for snapshot extraction + bank-snapshot
    // hardlinking to work.
    let (account_run_paths, _account_snapshot_paths) = create_all_accounts_run_and_snapshot_dirs(
        &[PathBuf::from(ledger_path).join(Path::new("stake-meta.accounts"))],
    )?;

    // Point the snapshot archive dirs at the ledger path (where a validator drops
    // `snapshot-<slot>-<hash>.tar.zst`). `SnapshotConfig::default()` is `usage: LoadOnly` with
    // archive generation disabled — exactly what we want for a read-only load.
    let snapshot_config = SnapshotConfig {
        full_snapshot_archives_dir: PathBuf::from(ledger_path),
        incremental_snapshot_archives_dir: PathBuf::from(ledger_path),
        bank_snapshots_dir: PathBuf::from(ledger_path).join(BANK_SNAPSHOTS_DIR),
        ..SnapshotConfig::default()
    };

    let exit = Arc::new(AtomicBool::new(false));

    // Prefer loading from a snapshot archive (O(1) — required for real clusters like devnet where
    // replaying from genesis over millions of slots is infeasible). Fall back to replaying from
    // genesis when no snapshot archive is present (e.g. a fresh local ledger).
    let (bank_forks, _) = match bank_forks_utils::try_load_bank_forks_from_snapshot(
        &genesis_config,
        &account_run_paths,
        &snapshot_config,
        &process_options,
        ...
        Some(loaded) => {
            info!("Loaded bank_forks from snapshot archive in {ledger_path:?}");
            loaded
        }
        None => {
            info!("No snapshot archive found; replaying from genesis to slot {snapshot_slot}");
    ...
    // The loaders above only establish the root bank (genesis or the snapshot slot) — they do NOT
    // replay the blockstore forward. Replay from the root up to `halt_at_slot` so the working bank
    // is at `snapshot_slot` (mirrors ledger-tool's `load_and_process_ledger`).
...
pub fn generate_stake_meta_collection(
    ...
    // The runtime no longer exposes `stakes_cache` to external crates, so reconstruct the
    // stake-delegation map by scanning all stake-program-owned accounts in the (frozen) bank.
    let delegations: imbl::HashMap<Pubkey, StakeAccount> = bank
        .get_program_accounts(&solana_stake_interface::program::id())
        .expect("scan stake program accounts")
        .into_iter()
        .filter_map(|(pubkey, account)| {
            StakeAccount::try_from(account).ok().map(|sa| (pubkey, sa))
        })
        .collect();
...
mod tests {
    use {
        ...
        solana_program::{message::Message, stake},

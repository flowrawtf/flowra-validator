...
/// `Transfer` instructions in `transaction`. Returns 0 if the transaction
/// does not write-lock any tip account.
fn transaction_tip_lamports(
    transaction: &impl TransactionWithMeta,
    tip_accounts: &HashSet<Pubkey>,
) -> u64 {
    let account_keys = transaction.account_keys();
    // Quick check: only walk instructions if some tip account is written to.
    if !(0..account_keys.len())
        .any(|index| transaction.is_writable(index) && tip_accounts.contains(&account_keys[index]))
    {
        return 0;
    }
    ...
    transaction
        ...
        .filter_map(|(_, instruction)| {
            ...
            tip_accounts.contains(destination).then(|| {
                u64::from_le_bytes(data[4..TRANSFER_DATA_LEN].try_into().unwrap())
            })
        })
...
fn combine_tip(mode: TipDamping, reward: u64, tip: u64) -> u64 {
    match mode {
        ...
        TipDamping::Cap { mult } => {
            reward.saturating_add(tip.min(reward.saturating_mul(mult)))
        }
    }
}
...
pub(crate) fn calculate_priority_and_cost(
    ...
    let mut reward = bank.calculate_reward_for_transaction(transaction, transaction_configuration);
